 ...
 */
	public function _readFormAttribute($attribute)
	{
 ...
 */
	public function inputRelationalManyChildren($options = array())
	{
		...
		$out .= $this->_orderedItens(compact(
			'texts','model_class_name','foreign_key', 'parameters',
			'allowed_content','baseID','callbacks', 'auto_order', 'url'
		));
 ...
 */
	public function inputRelationalEditableList($options = array())
	{
		...
		$options += array(
			...
			'assocName' => false,
			'url' => array('plugin' => 'burocrata', 'controller' => 'buro_burocrata', 'action' => 'autocomplete'),
			'allow' => array('create', 'modify', 'view', 'relate', 'unlink'),
			'baseID' => $this->baseID(),
			'texts' => array()
		);
		$options['texts'] += array(
		...
		if (!empty($this->data[$assocName]) && Set::numeric(array_keys($this->data[$assocName])))
		{
			...
			{
				$contents[] = array(
					'content' => $this->Jodel->insertModule($model_class_name, $type, array($model => $data)),
 ...
 */
	public function inputRelationalEditableListItem($options)
	{
		...
		if (in_array('unlink', $options['allow']))
