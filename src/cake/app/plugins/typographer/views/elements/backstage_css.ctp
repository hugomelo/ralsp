 ...
 */
	...
	$this->Decorator->rule(
		'.many_children .buro_form .input.buro h4, .content_stream .buro_form .input.buro  h4', array(
			'text-transform' => 'none'
	));
	...
	// form popup
	$this->Decorator->rule(
		'.box_popup.form_box p', array(
			'margin-bottom' => $vg->size('g')
		));

	$this->Decorator->rule(
		'.box_popup.form_box .buro_form label.buro, .box_popup.form_box .buro_form .input.buro h4', array(
			'text-transform' => 'none'
		));

	$this->Decorator->rule(
		'.box_popup.form_box .callbacks', array(
			'margin-top' => $vg->size('.5g')
		));

	$this->Decorator->rule(
		'.box_popup.form_box .alternative_option a', array(
			'text-transform' => 'none',
			'padding' => 0,
			'font-weight' => 'normal'
		));
	...
	$this->Decorator->rule(
		'a.link_button.disabled, a.link_button[disabled], a.link_button.disabled:hover, a.link_button[disabled]:hover', array(
			'color' => $palette['button_fg_disabled'] . ' !important',
...
#			'height' => $vg->size(array('g' => 6, 'u' => 2)),
	...
	$this->Decorator->rule(
		'#header .menu a.menu_item_1_user_administration', array(
			'float' => 'right',
	));
	...
	$this->Decorator->rule(
		'.control_box .alternative_option', array(
			'margin' => $vg->size('6m') . ' 0 ' . $vg->size('2m') . ' ' . $vg->size('m'),
		)
	);
	...
	$this->Decorator->rule(
		'.buro_form .mimic_disabled_input', array(
			'background' => '#FFF0FF',
			'font-family' => '"New Courrier", "Lucida Console", Monospace',
			'padding' => $u->t($padding_top) . ' ' . $u->t($padding_size),
			'margin-top' => $u->t($margin_top),
			'margin-bottom' => $u->t($margin_bottom),
			'width' => $u->t($hg->size('5M-g', false) - 2*$padding_size),
		)
	);
	...
	$this->Decorator->rule(
		'table.dashboard td .space', array(
			'text-transform' => 'uppercase',
			'font-size' => $u->t($line_height * 9/18),
	));
	...
	$this->Decorator->rule(
		'.lp_copy .lp_letter_summary', array(
			'background-color' => $palette['subform'],
			'width' => $hg->size('7M-g'),
			'padding' => $vg->size('g') . ' ' . $hg->size('.5g')
		)
	);
	
	$this->Decorator->rule(
		'.lp_copy .lp_letter_summary table', array(
			'width' => '100%'
		)
	);
	
	$this->Decorator->rule(
		'.lp_copy .lp_letter_summary table strong', array(
			'text-transform' => 'uppercase'
		)
	);
	
	$this->Decorator->rule(
		'.lp_copy .dash_toolbox, .lp_copy .filters', array(
			'display' => 'none'
		)
	);
	
	$this->Decorator->rule(
		'.lp_copy .control_panel', array(
			'float' => 'right',
		)
	);

	$this->Decorator->rule(
		'.lp_copy .control_panel a.link_button', array(
			'height' => 'auto',
			'line-height' => 'normal',
			'text-transform' => 'uppercase',
			'font-weight' => 'bold'
	...
	$this->Decorator->rule(
		'.lp_copy .lp_copy_header', array(
			'margin-left' => $hg->size('-g')
		)
	);
	
	$this->Decorator->rule(
		'.lp_copy .lp_copy_header hr', array(
			'border-top' => '1px solid black',
			'margin' => sprintf('%s 0', $vg->size('.5g'))
		)
	);
	
	$this->Decorator->rule(
		'.lp_copy .lp_copy_header h4', array(
			'text-transform' => 'uppercase'
		)
	);
	
	$this->Decorator->rule(
		'.lp_copy .lp_copy_header .preview, .lp_copy .lp_copy_header .preview *', array(
			'font-family' => '"New Courrier", "Lucida Console", Monospace'
		)
	);
	
	$this->Decorator->rule(
		'.lp_copy pre.preview', array(
			'white-space' => 'pre-wrap'
		)
	);
	
	$this->Decorator->rule(
		'.lp_copy .pagination', array(
			'top' => '-60px'
		)
	);
	
	$this->Decorator->rule(
		'.lp_copy .lp_copy_header .preview p', array('margin' => sprintf('%s 0', $vg->size('.5g')))
	);
	
	$this->Decorator->rule(
		'.lp_copy .lp_copy_header .preview ul', array('list-style-position' => 'inside')
	);

	//specific rules to sui admin backstage
	$this->Decorator->rule(
		'.sui_application .dash_header .right', array(
			'float' => 'right'
	));
	
	$this->Decorator->rule(
		'.sui_application .dash_header .left', array(
			'float' => 'left'
	));
	
	$this->Decorator->rule(
		'.sui_application h2', array(
			'margin-bottom' => $vg->size(array('m' => 1))
	));
	
	$this->Decorator->rule(
		'.sui_user .dash_search .buro.input', array(
			'float' => 'left',
			'width' => '100% !important'
	));
	
	$this->Decorator->rule(
		'.sui_user .dash_search, .sui_institution .dash_search', array(
			'width' => 'auto !important'
	));
	
	$this->Decorator->rule(
		'.sui_user .dash_search select.buro, #backstage_states_id, #backstage_cities_id', array(
			'width' => '100% !important'
	));
	
	$this->Decorator->rule(
		'.sui_application .sui_application_links', array(
			'margin-top' => $vg->size('1g-m')
	));
	
	$this->Decorator->rule(
		'.sui_user .sui_user_sheet', array(
			'margin-top' => $vg->size('1g-m')
	));
	
	$this->Decorator->rule(
		'.sui_institution #backstage_custom_table', array(
			'margin-top' => $vg->size('2g')
	));
	
	$this->Decorator->rule(
		'.sui_application #backstage_custom_table, .sui_user #backstage_custom_table, 
		.user_profiles  #backstage_custom_table, .user_users  #backstage_custom_table,
		.user_permissions  #backstage_custom_table', array(
			'margin-top' => $vg->size('1gm')
	));
	
	// Custom paginations
	$this->Decorator->rule(
		'.sui_institution  #backstage_custom_table .pagination, .sui_user  #backstage_custom_table .pagination', array(
			'top' => '-40px'
		)
	);
	
	$this->Decorator->rule(
		'.sui_application  #backstage_custom_table .pagination', array(
			'top' => '-8px'
		)
	);
	
	$this->Decorator->rule(
		'.sui_application .dash_filter #SuiApplicationCode, .sui_application .dash_filter label[for=SuiApplicationCode]', array(
			'width' => $hg->size('2M')
	)); 
	
	$this->Decorator->rule(
		'.sui_application .dash_filter .dash_search', array(
			'width' => 'auto'
	)); 
	
	$this->Decorator->rule(
		'.user_profiles  #backstage_custom_table .pagination, .user_users  #backstage_custom_table .pagination, .user_permissions  #backstage_custom_table .pagination', array(
			'top' => '-58px'
		)
	);
	
	$this->Decorator->rule(
		'.user_profiles  div.dash_toolbox, .user_users  div.dash_toolbox, .user_permissions  div.dash_toolbox', array(
			'float' => 'none'
		)
	);
	...
	$this->Decorator->rule(
		'.data_popin h3', array(
			'font-size' => $u->t(14),
			'display' => 'block',
			'margin-bottom' => $vg->size('2m')
		)
	);
	
	$this->Decorator->rule(
		'.content_data_popin', array(
			'margin-left' => $hg->size('-g')
		)
	);
	
	$this->Decorator->rule(
		'.data_popin .content_data_popin span', array(
			'word-wrap' => 'break-word'
		)
	);
	
	$this->Decorator->rule(
		'.data_popin .content_data_popin', array(
			'margin-top' => $vg->size('g')
		)
	);
	$this->Decorator->rule(
		'.data_popin .content_data_popin.second', array(
			'margin-top' => $vg->size('-g')
		)
	);
	$this->Decorator->rule(
		'.data_popin .content_data_popin > adiv', array(
			'margin-top' => $vg->size('m'),
			'float' => 'left',
			'margin-bottom' => $vg->size('0.5g'),
			'padding-bottom' => $vg->size('g'),
		)
	);
	$this->Decorator->rule(
		'.data_popin .content_data_popin div.header_table', array(
			'margin-bottom' => $vg->size('m'),
			'padding-bottom' => $u->t(0)
		)
	);
	$this->Decorator->rule(
		'.data_popin .content_data_popin div.row_data', array(
			'margin-bottom' => $vg->size('m'),
			'padding-bottom' => $u->t(0)
		)
	);
	$this->Decorator->rule(
		'.data_popin .content_data_popin div.column', array(
			'float' => 'left',
			'margin-right' => $hg->size('g'),
			'width' => $hg->size('3M-3g')
		)
	...
	$this->Decorator->rule(
		'.data_popin .content_data_popin div.column2', array(
			'float' => 'left',
			'margin-right' => $hg->size('g'),
			'width' => $hg->size('2M-3g')
		)
	...
	$this->Decorator->rule(
		'.data_popin .content_data_popin div.column3', array(
			'float' => 'left',
			'margin-right' => $hg->size('g'),
			'width' => $hg->size('M-3g')
		)
	...
	$this->Decorator->rule(
		'.data_popin .content_data_popin div label', array(
			'font-weight' => 'bold',
			'display' => 'block',
			'font-size' => $u->t(8),
			'line-height' => $u->t(11),
