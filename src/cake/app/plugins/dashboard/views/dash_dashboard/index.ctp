		...
		echo $this->Bl->sdiv(array('class' => array('dash_filter')));
			echo $this->Bl->sdiv(array('id' => 'dash_filter_list'));
				...
				echo $this->Bl->sdiv(array('class' => 'filters'));
					...
					{
						$can_view = true;
						if (!$this->JjAuth->can($space['MexcSpace']['id']))
						{
							$can_view = false;
						}
						if ($can_view)
						{
							$filterLink = $ajax->link($space['MexcSpace']['name'], 			
								array(
									'plugin' => 'dashboard',
								...
								array(
									'before' => "$('dashboard_table').setLoading();",
									'complete' => "$('dashboard_table').unsetLoading();",
									'id' => 'filter_space_'.$space['MexcSpace']['id'],
									'class' => $space['MexcSpace']['id'],
									'update' => 'dashboard_table'
								)
							);
							$selected = $filter_space == $space['MexcSpace']['id'] ? 'true' : 'false';
							$this->BuroOfficeBoy->addHtmlEmbScript("new FilterLink('filter_space_{$space['MexcSpace']['id']}', $selected);");
						}
						else
						{
							$filterLink = $ajax->link(
								$space['MexcSpace']['name'],
								array(),
								array(
									'class'	=> 'disabled',
									'onclick' => "return false;",
						...
						<style type="text/css" media="screen">
							#dash_filter_list #filter_space_'.$space['MexcSpace']['id'].':hover {
								color: '.$space['MexcSpace']['fore_color'].' !important;
								background-color: '.$space['MexcSpace']['back_color'].' !important;
							}
							
							#dash_filter_list #filter_space_'.$space['MexcSpace']['id'].'.selected {
								color: '.$space['MexcSpace']['fore_color'].' !important;
								background-color: '.$space['MexcSpace']['back_color'].' !important;
							}
						</style> ';

						$this->addScript($space['MexcSpace']['id'],  $style); 

					}
					$filterLink = $ajax->link(__d('dashboard','Show All', true), 			
						array(
							'plugin' => 'dashboard','controller' => 'dash_dashboard','action' => 'filter_space','all'
						), 
						array(
							'before' => "$('dashboard_table').setLoading();",
							'complete' => "$('dashboard_table').unsetLoading();",
					...
					$selected = $filter_space == 'all' || empty($filter_space) ? 'true' : 'false';
					$this->BuroOfficeBoy->addHtmlEmbScript("new FilterLink('filter_space_all', $selected);");
					
					echo $this->Bl->h4Dry(__d('dashboard','Spaces', true));
					echo $this->Text->toList($linkFilters, ' ', ' ');
				echo $this->Bl->ediv();
				echo $this->Bl->floatBreak();
