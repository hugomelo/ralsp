 ...
 */
	...
	echo $this->Bl->ssmartTable(array('class' => 'dashboard'), array(
		...
		$smartTableColumns[] = $this->Paginator->sort(__d('dashboard','Dashboard header: Space',true),'mexc_space_id');
		...
		foreach ($this->data as $k => $item)
		{
			...
			$smartTableRow[] = __d('dashboard', $type, true);
				$smartTableRow[] = $this->Bl->span(
					array(
						'class' => 'space '.$item['MexcSpace']['id'],
						'style' => "color:".$item['MexcSpace']['fore_color']."; background-color:".$item['MexcSpace']['back_color']
					), 
					array(), 
					$item['MexcSpace']['name']
				);
			...
			echo $this->Bl->smartTableRowDry(array(
					array(array(),array('escape' => false, 'colspan' => 4),$links)
			));
