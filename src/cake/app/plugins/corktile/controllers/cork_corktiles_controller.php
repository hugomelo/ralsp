...
class CorkCorktilesController extends CorktileAppController {
	...
	function edit($key)
	{
		...
		// Needed for mexc spaces
		StatusBehavior::setGlobalActiveStatuses(array(
			'publishing_status' => array('active' => array('published', 'draft'), 'overwrite' => true),
		));
		
		if (isset($this->data['CorkCorktile']['location'][2]))
		{
			$this->currentSpace = $this->data['CorkCorktile']['location'][2];
			...
			$this->SectSectionHandler->initialize($this);
		}
		// End of Mexc specific code
